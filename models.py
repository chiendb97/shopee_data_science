...
class RobertaForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, activation_function, loss_type):
        ...
        self.classifier_ner = nn.Linear(4 * config.hidden_size, config.num_labels)
    ...
    def forward(
        ...
        if self.activation_function == 'softmax':
            ...
            if labels_ner is not None:
                ...
                if attention_mask is not None:
                    ...
                    active_logits = logits_ner.view(-1, self.num_labels)
                    ...
                    loss_ner = loss_fct(logits_ner.view(-1, self.num_labels), labels_ner.view(-1))
        ...
        return (logits_ner, loss_ner) if loss_ner is not None else logits_ner
...
class RobertaForClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, loss_type):
        ...
        self.roberta = RobertaModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier_cf = nn.Linear(4 * config.hidden_size, config.num_labels)
    ...
    def forward(
        ...
        return (logits_cf, loss_cf) if loss_cf is not None else logits_cf
